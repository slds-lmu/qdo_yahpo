...
def run_ranger_interpretability(task_id, emitter_type, seed):
...
def run_ranger_hardware(task_id, emitter_type, seed):
    bench = benchmark_set.BenchmarkSet("iaml_ranger", check = False)
    bench.set_instance(task_id)
    search_space = bench.get_opt_space()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description = "Benchmark ranger")
    parser.add_argument("context", choices=["interpretability", "hardware"])
    args = parser.parse_args()
    if args.context == "interpretability":
        run_f = run_ranger_interpretability
    elif args.context == "hardware":
        run_f = run_ranger_hardware
    repls = 10
    for task_id in ["41146", "40981", "1489", "1067"]:
        df_task = [None] * repls
        for repl in range(repls):
            random = run_f(task_id, "random", repl)
            random["method"] = "random"
            gaussian = run_f(task_id, "gaussian", repl)
            gaussian["method"] = "gaussian"
            improvement = run_f(task_id, "improvement", repl)
            improvement["method"] = "improvement"
            mixed = run_f(task_id, "mixed", repl)
            mixed["method"] = "mixed"
        ...
        res_task.to_csv("Results/ranger_" + args.context + "_" + task_id + ".csv", index = False)
